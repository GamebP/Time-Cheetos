...
namespace ZeroGUI
{
	namespace Colors
	{
		...
		static ue::FLinearColor Window_Header{ 0.21f, 0.20f, 0.21f, 1.00f };
	...
	void Checkbox(const char* name, bool* value)
	{
		...
		if (isHovered)
		{
			drawFilledRect(Vector2{ pos.x, pos.y }, size, size, ue::FLinearColor(0.0f, 0.0f, 0.0f, 0.30f));
			hover_element = true;
		}
		else
		{
			drawFilledRect(Vector2{ pos.x, pos.y }, size, size, ue::FLinearColor(0.21f, 0.20f, 0.21f, 1.00f));
		}
		...
		if (*value)
		{
			drawFilledRect(Vector2{ pos.x + 3, pos.y + 3 }, size - 6, size - 6, ue::FLinearColor(1.0f, 1.0f, 1.0f, 1.0f));
	...
	static void SliderFloat(const char* name, float* value, float min, float max, const char* format = "%.0f")
	{
		...
		//Value
		drawFilledRect(Vector2{ pos.x, pos.y + slider_size.y + padding.y }, oneP * (*value - min), slider_size.y, ue::FLinearColor(1.0f, 1.0f, 1.0f, 1.0f));
